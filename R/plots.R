...
plot(df$g,df$b, col=col_r, pch=19, cex=1,xlab="Green", ylab="Blue")

col_g <- rgb(0,df$g/255.0,0)
plot(df$r,df$b, col=col_g, pch=19, cex=1,xlab="Red", ylab="Green")

col_b<- rgb(0,0,df$b/255.0)
plot(df$r,df$g, col=col_b, pch=19, cex=1,,xlab="Red", ylab="Blue")
...
# animation
 ...
 plot_ly(df,x=g,y=b, mode="markers",
         marker = list(color =as.vector(as.character(col_r))),title="Colors in Animation Movie Posters")
